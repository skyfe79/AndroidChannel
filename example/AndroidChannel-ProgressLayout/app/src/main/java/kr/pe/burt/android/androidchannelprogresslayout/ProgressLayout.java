...
/**
 * Reimplement ProgressLayout( https://github.com/iammert/ProgressLayout ) by using AndroidChannel.
 * It shows how to use AndroidChannel when you implement a custom animatable view.
 * Created by burt on 15. 10. 8..
 */
public class ProgressLayout extends View implements Animatable {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        progressChannel = new Channel(new Channel.UiCallback() {
            @Override
                ...
                if(isPlaying) {
                    if(currentProgress >= maxProgress) {
                        ...
                        if(progressLayoutListener != null) {
                            progressChannel.toUI().post(new Runnable() {
                                @Override
                                public void run() {
                        ...
                        if(progressLayoutListener != null) {
                            progressChannel.toWorker().post(new Runnable() {
                                @Override
                                public void run() {
                        ...
                        progressChannel.toWorker().sendEmptyMessageDelayed(0, PROGRESS_SECOND_MS/10);
                    }
                }
                return false;
            }
    ...
    public void start() {
        ...
        progressChannel.toWorker().sendEmptyMessage(0);
    }
